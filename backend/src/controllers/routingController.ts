...
async function createSyntheticTransitRoutes(walkingRoute: any, origin: string, destination: string) {
...
export default {
  ...
  getRoutes: async (req: Request, res: Response) => {
    try {
      ...
      const walkingResponse = await axios.get(mapboxDirectionsUrl, {
        params: {
      ...
      const syntheticRoutes = await createSyntheticTransitRoutes(walkingRoute, origin as string, destination as string);
 ...
 */
async function enhanceRoutesWithTransitData(routes: any[]) {
 ...
 */
function extractTransitInfo(instruction: string) {
 ...
 */
function extractStops(instruction: string) {
 ...
 */
function categorizeRoutesByTransitType(routes: any[]) {
  const busRoutes: any[] = [];
  const trainRoutes: any[] = [];
  const mixedRoutes: any[] = [];
